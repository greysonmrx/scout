      ...
      <Wrapper>
        ...
        {!list ? (
          ...
          <>
              ...
              <main>
                <div className="avatar">
                  {list.player ? (
                    <>
                      <img
                        src={`http://147.182.206.193:5000/files/${list.player.avatar.path}`}
                        alt={list.player.name}
                      />
                      <Link to={`/players/details/${list.player.id}`}>
                        {list.name}
                      </Link>
                    </>
                  ) : (
            ...
            <h3>
              {
                list.player ? "Substitutos" : "Opções"
              }
            </h3>
              ...
              {list.items.map((substitute) => (
                <div className="player-details" key={substitute.player.id}>
                  <main>
                    <div className="avatar">
                      <img
                        src={`http://147.182.206.193:5000/files/${substitute.player.avatar.path}`}
                        alt={substitute.player.name}
                      />
                      <Link to={`/players/details/${substitute.player.id}`}>
